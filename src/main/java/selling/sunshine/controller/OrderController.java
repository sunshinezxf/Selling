...
public class OrderController {
	...
	public ModelAndView express(HttpServletRequest request) {
		...
		List<Express> expressList = new ArrayList<>();
		for (int i = 0; i < orderItems.size(); i++) {
			Express express = new Express("代填", "云草纲目", "18000000000", "云南",
					orderItems.get(i).getCustomer().getName(), orderItems
							.get(i).getCustomer().getPhone().getPhone(),
					orderItems.get(i).getCustomer().getAddress().getAddress(),
					orderItems.get(i).getGoods().getName(), expressDate);
	...
	public ResultData express(@RequestBody List<ExpressForm> expressData) {
		...
		for (Express express : expressList) {
			...
			if (express.getOrderItem() != null) {
				...
				Map<String, Object> condition = new HashMap<>();
				condition.put("orderItemId", express.getOrderItem()
						.getOrderItemId());
