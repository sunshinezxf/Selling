...
/**
 * 代理商的信息相关的所有请求入口 参数列表中的code, state为通过微信Oauth2.0鉴权所携带的参数 Created by sunshine on
 * 4/8/16.
 */
@RestController
...
public class AgentController {
    ...
    public ModelAndView register(@Valid AgentForm form, BindingResult result, HttpServletRequest request) {
        ...
        try {
            ...
            // 根据用户提交的表单构造代理信息
            Agent agent = new Agent(form.getName(), form.getGender(), form.getPhone(), form.getAddress(),
                    form.getCard(), form.getPassword(), form.getWechat(), form.getWechat_id(),
                    StringUtils.isEmpty(form.getMemberNum()) ? 0 : Integer.parseInt(form.getMemberNum()));
            if (!StringUtils.isEmpty(form.getUpper())) {
                ...
                if (agentResponse.getResponseCode() == ResponseCode.RESPONSE_OK) {
                    agent.setUpperAgent(new common.sunshine.model.selling.agent.lite.Agent(
                            ((List<Agent>) agentResponse.getData()).get(0)));
                }
            }
            ...
            if ((!form.getFront().startsWith("/material") && !form.getFront().startsWith("\\material"))
                    || (!form.getBack().startsWith("/material") && !form.getBack().startsWith("\\material"))) {
                // 进行微信上传身份证的操作
                String front = "";
                String back = "";
                if (!form.getFront().startsWith("/material") && !form.getFront().startsWith("\\material")) {
                    front = WechatUtil.downloadCredit(form.getFront(), PlatformConfig.getAccessToken(),
                            request.getSession().getServletContext().getRealPath("/"));
                } else {
                    front = form.getFront();
                }
                if (!form.getBack().startsWith("/material") && !form.getBack().startsWith("\\material")) {
                    back = WechatUtil.downloadCredit(form.getBack(), PlatformConfig.getAccessToken(),
                            request.getSession().getServletContext().getRealPath("/"));
                } else {
                    back = form.getBack();
                }
                ...
                // 进行非微信上传身份证的操作
                Credit credit = new Credit(form.getFront(), form.getBack(),
                        new common.sunshine.model.selling.agent.lite.Agent(agent));
                if (form.getAgentId() != null && !form.getAgentId().equals("")) {
                    agentService.updateCredit(credit);
                } else {
                    agentService.createCredit(credit);
    ...
    public ModelAndView placeOrder(String code) {
        ...
        if (agent.isGranted()) {
            ...
            Prompt prompt = new Prompt(PromptCode.WARNING, "提示", "尊敬的代理商，您的账号信息未被审核通过，请重新修改账号信息！",
                    "/agent/registermodify");
            view.addObject("prompt", prompt);
            WechatConfig.oauthWechat(view, "/agent/prompt");
            view.setViewName("/agent/prompt");
            return view;
    ...
    public ModelAndView placeOrder(@Valid OrderItemForm form, BindingResult result, RedirectAttributes attr,
        ...
        for (int i = 0; i < length; i++) {
            ...
            OrderItem orderItem = new OrderItem(customerId, goodsId, goodsQuantity, orderItemPrice, address,
                    description);// 构造OrderItem
            ...
            if (orderItemSum.getOrderType() != OrderType.GIFT) {
                ...
                for (OrderSumOverview orderSumOverview : buyList) {
                    if (orderSumOverview.getGoods().getGoodsId().equals(orderItemSum.getGoods().getGoodsId())) {
                        ...
                        orderSumOverview
                                .setTotalPrice(orderSumOverview.getTotalPrice() + orderItemSum.getOrderItemPrice());
                        break;
                    }
                }
                if (!find) {
                ...
                if (orderTime.startsWith(month)) {
                    for (OrderSumOverview orderSumOverview : monthList) {
                        if (orderSumOverview.getGoods().getGoodsId().equals(orderItemSum.getGoods().getGoodsId())) {
                            find = true;
                            orderSumOverview
                                    .setQuantity(orderSumOverview.getQuantity() + orderItemSum.getGoodsQuantity());
                            orderSumOverview
                                    .setTotalPrice(orderSumOverview.getTotalPrice() + orderItemSum.getOrderItemPrice());
                            break;
                        }
    ...
    public ModelAndView viewOrder(@PathVariable("orderId") String orderId) {
        ...
        if (orderId.startsWith("ODR")) {
            ...
            ResultData fetchOrderResponse = orderService.fetchOrder(condition);
            if (fetchOrderResponse.getResponseCode() != ResponseCode.RESPONSE_OK
                    || ((List<Order>) fetchOrderResponse.getData()).isEmpty()) {
            ...
            if (fetchCustomerOrderResponse.getResponseCode() != ResponseCode.RESPONSE_OK
                    || ((List<CustomerOrder>) fetchCustomerOrderResponse.getData()).isEmpty()) {
                Prompt prompt = new Prompt(PromptCode.WARNING, "提示信息", "未找到该订单", "/agent/manage/2");
                view.addObject("prompt", prompt);
                WechatConfig.oauthWechat(view, "/agent/prompt");
                view.setViewName("/agent/prompt");
    ...
    public ModelAndView refundConfig() {
        ModelAndView view = new ModelAndView();
        // Map<String, Object> condition = new HashMap<String, Object>();
        // ResultData fetchRefundData =
        // refundService.fetchRefundConfig(condition);
        // if (fetchRefundData.getResponseCode() != ResponseCode.RESPONSE_OK) {
        // return view;
        // }
        // List<RefundConfig> configs = (List<RefundConfig>)
        // fetchRefundData.getData();
    ...
    public ModelAndView modifyPassword(@Valid PasswordForm form, BindingResult result) {
        ModelAndView view = new ModelAndView();
        if (result.hasErrors() || StringUtils.isEmpty(form.getPassword())
    ...
    public ModelAndView grantNotPass(@PathVariable("agentId") String agentId, HttpServletRequest request) {
        ...
        if (fetchResponse.getResponseCode() == ResponseCode.RESPONSE_OK) {
            ...
            Admin admin = user.getAdmin();
            BackOperationLog backOperationLog = new BackOperationLog(admin.getUsername(), toolService.getIP(request),
                    "管理员" + admin.getUsername() + "审核代理商" + agent.getName() + ",手机：" + agent.getPhone() + "不通过");
            logService.createbackOperationLog(backOperationLog);
        }
        view.setViewName("redirect:/agent/overview");
        return view;
    ...
    // "/setUpperAgent/{agentId}/{upperAgentId}")
    // public ModelAndView setUpperAgent(@PathVariable("agentId") String
    // agentId, @PathVariable("upperAgentId") String upperAgentId,
    ...
    // agentService.fetchAgent(condition).getData()).get(0);
    // common.sunshine.model.selling.agent.lite.Agent upperAgent = new
    // common.sunshine.model.selling.agent.lite.Agent();
    ...
    // Admin admin = user.getAdmin();
    // if (fetchResponse.getResponseCode() == ResponseCode.RESPONSE_OK) {
    // Agent agent2 = ((List<Agent>)
    // agentService.fetchAgent(condition).getData()).get(0);
    // BackOperationLog backOperationLog = new BackOperationLog(
    // admin.getUsername(), toolService.getIP(request), "管理员" +
    // admin.getUsername() + "设置了代理商" + agent.getName() + ",手机：" +
    // agent.getPhone() + "的上级代理商为" + agent2.getName());
    // logService.createbackOperationLog(backOperationLog);
    // }
    //
    // }
    ...
    // Map<String, Object> condition = new HashMap<>();
    // condition.put("agentId", agentId);
    // if (agentService.fetchAgent(condition).getResponseCode() !=
    ...
    // condition.put("granted", true);
    // condition.put("blockFlag", false);
    // List<Agent> agentList = (List<Agent>)
    // agentService.fetchAgent(condition).getData();
    // Iterator<Agent> iter = agentList.iterator();
    // while (iter.hasNext()) {
    // if (iter.next().getAgentId().equals(agent.getAgentId())) {
    // iter.remove();
    ...
    public ResultData modifyUpperAgent(@PathVariable("agentId") String agentId,
        ...
        if (fetchResponse.getResponseCode() == ResponseCode.RESPONSE_OK) {
            ...
            if (fetchResponse.getResponseCode() == ResponseCode.RESPONSE_OK) {
                ...
                Admin admin = user.getAdmin();
                BackOperationLog backOperationLog = new BackOperationLog(admin.getUsername(),
                        toolService.getIP(request), "管理员" + admin.getUsername() + "修改了代理商" + agent.getName() + ",手机："
                        + agent.getPhone() + "的上级代理商为" + agent2.getName());
                logService.createbackOperationLog(backOperationLog);
            }
        }
    ...
    public ModelAndView forbid(@PathVariable("agentId") String agentId, HttpServletRequest request) {
        ...
        Admin admin = user.getAdmin();
        BackOperationLog backOperationLog = new BackOperationLog(admin.getUsername(), toolService.getIP(request),
                "管理员" + admin.getUsername() + "禁用了代理商" + agent.getName() + ",手机：" + agent.getPhone());
        logService.createbackOperationLog(backOperationLog);
        view.setViewName("redirect:/agent/overview");
        return view;
    }
    ...
    /**
     * 此方法需与页面上的列表每一列的内容想对应，该方法目前对应的页面上的列为: 代理商姓名 | 代理商类型 | 上级代理 | 返现金额 | 注册时间
     *
     * @param param
     * @return
     */
    ...
    public DataTablePage<Agent> list(DataTableParam param) {
        String[] method = new String[]{"convert(agent_name using gbk)", "agent_type", "agent_refund", "create_time"};
        ...
        if (!StringUtils.isEmpty(param.getsSortDir_0())) {
            ...
            switch (param.getiSortCol_0()) {
                case 0:
                    item = new SortRule(method[0], param.getsSortDir_0());
                    rule.add(item);
                    break;
                case 1:
                    item = new SortRule(method[1], param.getsSortDir_0());
                    rule.add(item);
                    break;
                case 3:
                    item = new SortRule(method[2], param.getsSortDir_0());
                    rule.add(item);
                    break;
                case 4:
                    item = new SortRule(method[3], param.getsSortDir_0());
                    rule.add(item);
                    break;
                default:
                    break;
    ...
    public String downloadAgentExcel(HttpServletRequest request, HttpServletResponse response, String start, String end)
        ...
        response.setHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode("代理商.xls", "utf-8"));
        ...
        if (agentList != null) {
            ...
            for (int i = 0; i < agentList.size(); i++) {
                ...
                sheet1.addCell(
                        new Label(8, k, agent.getUpperAgent() == null ? "无" : agent.getUpperAgent().getAgentId()));
                ...
                for (int m = 0; m < 3; m++) {
                    for (int n = 0; n < goodsList.size(); n++) {
                        ...
                        if (fetchOrderPoolData.getResponseCode() == ResponseCode.RESPONSE_OK
                                && !((List<OrderPool>) fetchOrderPoolData.getData()).isEmpty()) {
    ...
    public ModelAndView agentVitalityConfig(@PathVariable("agentVitalityId") String agentVitalityId,
        ...
        AgentVitality agentVitality = new AgentVitality(agentVitalityId, Integer.parseInt(form.getVitalityQuantity()),
                Double.parseDouble(form.getVitalityPrice()));
        ...
        if (resultData.getResponseCode() == ResponseCode.RESPONSE_OK) {
            ...
            Admin admin = user.getAdmin();
            BackOperationLog backOperationLog = new BackOperationLog(admin.getUsername(), toolService.getIP(request),
                    "管理员" + admin.getUsername() + "修改了代理商活跃度配置");
            logService.createbackOperationLog(backOperationLog);
    ...
    public ModelAndView contributionFactorView() {
        ...
        if (fetchResponse.getResponseCode() == ResponseCode.RESPONSE_OK) {
            ...
            for (ContributionFactor contributionFactor : list) {
                if (contributionFactor.getFactorId().equals("FAC00000001")) {
                    view.addObject("quantity", contributionFactor);
                } else if (contributionFactor.getFactorId().equals("FAC00000002")) {
                    view.addObject("price", contributionFactor);
                } else if (contributionFactor.getFactorId().equals("FAC00000003")) {
                    view.addObject("directAgentQuantity", contributionFactor);
                }
            }
        }
    ...
    public ModelAndView updateContributionFactor(@Valid ContributionFactorForm form, BindingResult result,
        ...
        ModelAndView view = new ModelAndView();
        if (result.hasErrors()) {
            view.setViewName("redirect:/agent/contributionFactor");
            return view;
        }
        if (StringUtils.isEmpty(form.getQuantityWeight())) {
            form.setQuantityWeight("0");
        }
        if (StringUtils.isEmpty(form.getPriceWeight())) {
            form.setPriceWeight("0");
        }
        if (StringUtils.isEmpty(form.getDirectAgentQuantityWeight())) {
            form.setDirectAgentQuantityWeight("0");
        }
        Map<String, Object> condition = new HashMap<>();
        ResultData fetchResponse = new ResultData();
        fetchResponse = contributionFactorService.fetchContributionFactor(condition);
        if (fetchResponse.getResponseCode() == ResponseCode.RESPONSE_OK) {
            ...
            for (ContributionFactor contributionFactor : list) {
                if (contributionFactor.getFactorId().equals("FAC00000001")) {
                    contributionFactor.setFactorWeight(Double.parseDouble(form.getQuantityWeight()));
                } else if (contributionFactor.getFactorId().equals("FAC00000002")) {
                    contributionFactor.setFactorWeight(Double.parseDouble(form.getPriceWeight()));
                } else if (contributionFactor.getFactorId().equals("FAC00000003")) {
                    contributionFactor.setFactorWeight(Double.parseDouble(form.getDirectAgentQuantityWeight()));
                }
                contributionFactorService.updateContributionFactor(contributionFactor);
            }
        }
        view.setViewName("redirect:/agent/contributionFactor");
        return view;
